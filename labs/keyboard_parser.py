#!/usr/bin/env python3
import struct
import sys

# HID Usage IDs & modifier bits per HID Usage Tables 1.12 (§10)
hid_map = {
    'a': (0x00, 0x04), 'b': (0x00, 0x05), 'c': (0x00, 0x06), 'd': (0x00, 0x07),
    'e': (0x00, 0x08), 'f': (0x00, 0x09), 'g': (0x00, 0x0A), 'h': (0x00, 0x0B),
    'i': (0x00, 0x0C), 'j': (0x00, 0x0D), 'k': (0x00, 0x0E), 'l': (0x00, 0x0F),
    'm': (0x00, 0x10), 'n': (0x00, 0x11), 'o': (0x00, 0x12), 'p': (0x00, 0x13),
    'q': (0x00, 0x14), 'r': (0x00, 0x15), 's': (0x00, 0x16), 't': (0x00, 0x17),
    'u': (0x00, 0x18), 'v': (0x00, 0x19), 'w': (0x00, 0x1A), 'x': (0x00, 0x1B),
    'y': (0x00, 0x1C), 'z': (0x00, 0x1D),
    'A': (0x02, 0x04), 'B': (0x02, 0x05), 'C': (0x02, 0x06), 'D': (0x02, 0x07),
    'E': (0x02, 0x08), 'F': (0x02, 0x09), 'G': (0x02, 0x0A), 'H': (0x02, 0x0B),
    'I': (0x02, 0x0C), 'J': (0x02, 0x0D), 'K': (0x02, 0x0E), 'L': (0x02, 0x0F),
    'M': (0x02, 0x10), 'N': (0x02, 0x11), 'O': (0x02, 0x12), 'P': (0x02, 0x13),
    'Q': (0x02, 0x14), 'R': (0x02, 0x15), 'S': (0x02, 0x16), 'T': (0x02, 0x17),
    'U': (0x02, 0x18), 'V': (0x02, 0x19), 'W': (0x02, 0x1A), 'X': (0x02, 0x1B),
    'Y': (0x02, 0x1C), 'Z': (0x02, 0x1D),
    '0': (0x00, 0x27), '1': (0x00, 0x1E), '2': (0x00, 0x1F), '3': (0x00, 0x20),
    '4': (0x00, 0x21), '5': (0x00, 0x22), '6': (0x00, 0x23), '7': (0x00, 0x24),
    '8': (0x00, 0x25), '9': (0x00, 0x26),
    '{': (0x02, 0x2F), '}': (0x02, 0x30), '_': (0x02, 0x2D),
}

# Invert for lookup: (modifier, keycode) → character
inv_map = {v: k for k, v in hid_map.items()}

PCAP_GLOBAL_HDR_LEN  = 24
PCAP_PKT_HDR_LEN    = 16
USBMON_PKT_HDR_FMT  = "<QBBBBHbbqiiII8s"  # exactly 48 bytes
USBMON_PKT_HDR_LEN  = struct.calcsize(USBMON_PKT_HDR_FMT)
LINKTYPE_USB_LINUX  = 189

def parse_usb_hid_pcap(path):
    out_chars = []
    with open(path, "rb") as f:
        # Read & validate global header
        gh = f.read(PCAP_GLOBAL_HDR_LEN)
        _, _, _, _, _, _, network = struct.unpack("=IHHIIII", gh)
        if network != LINKTYPE_USB_LINUX:
            print(f"[!] Warning: DLT={network}, expected {LINKTYPE_USB_LINUX}")

        # Iterate packets
        while True:
            ph = f.read(PCAP_PKT_HDR_LEN)
            if len(ph) < PCAP_PKT_HDR_LEN:
                break
            ts_sec, ts_usec, incl_len, orig_len = struct.unpack("=IIII", ph)
            data = f.read(incl_len)
            if len(data) != USBMON_PKT_HDR_LEN:
                # skip malformed or unexpected-size packets
                continue

            # Unpack the 48-byte usbmon_packet header + 8-byte HID report
            fields = struct.unpack(USBMON_PKT_HDR_FMT, data)
            hid_report = fields[-1]            # last field = 8s
            modifier, _, keycode = hid_report[0], hid_report[1], hid_report[2]

            # Ignore key‐releases
            if keycode == 0:
                continue

            ch = inv_map.get((modifier, keycode))
            if ch:
                out_chars.append(ch)
            else:
                out_chars.append(f"[0x{modifier:02x},0x{keycode:02x}]")

    return "".join(out_chars)

def main():
    if len(sys.argv) != 2:
        print(f"Usage: {sys.argv[0]} usb_keyboard_fixed.pcap")
        sys.exit(1)
    pcap_path = sys.argv[1]
    sequence = parse_usb_hid_pcap(pcap_path)
    print("Reconstructed HID sequence:")
    print(sequence)

if __name__ == "__main__":
    main()
